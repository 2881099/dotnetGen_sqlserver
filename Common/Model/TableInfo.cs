...
namespace Model {
	...
	public class TableInfo {
		...
		public TableInfo(int id, string owner, string name, string type) {
			_id = id;
			_owner = owner;
			_name = name;
			_Type = type;
		...
		public static string GetClassName(string name) {
			int rr = 0;
			string n = name.StartsWith("dbo.") ? name.Substring(4) : Regex.Replace(name, @"\.", delegate(Match m) {
		...
		public string FullName {
			get { return string.Format("{0}.{1}", _owner, _name); }
		}
		public string Type {
			get { return _Type; }
		}
