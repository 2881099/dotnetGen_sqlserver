		...
		public List<BuildInfo> Build(string solutionName, bool isSolution, bool isMakeAdmin, bool isDownloadRes) {
			...
			item.{0} = newitem.{0};", UFString(columnInfo.Name));
					if (columnInfo.IsIdentity) {
						//CsParamNoType2 += "null, ";
					} else if (columnInfo.IsPrimaryKey && CodeBuild.GetCSType(columnInfo.Type) == "Guid?" && table.PrimaryKeys.Count == 1) {
				...
				foreach (List<ColumnInfo> cs in table.Uniques) {
					...
					foreach (ColumnInfo columnInfo in cs) {
						...
						whereCondi += string.Format(".Where{0}({1})", UFString(columnInfo.Name),
							GetCSType(columnInfo.Type).Contains("?") && !cs[0].IsPrimaryKey ? string.Concat("new ", GetCSType(columnInfo.Type), "(", UFString(columnInfo.Name), ")") : UFString(columnInfo.Name));
					}
					parms = parms.Substring(0, parms.Length - 2);
		...
		async public static Task<int> Delete{2}Async({0}) {{
			...
			return affrows;
					...
					*/
					...
					sb3.AppendFormat(@"
		public static {1}Info GetItem{2}({4}) => SqlHelper.CacheShell(string.Concat(""{0}_BLL_{1}{2}_"", {3}), itemCacheTimeout, () => Select{7}.ToOne());", solutionName, uClass_Name, cs[0].IsPrimaryKey ? string.Empty : parmsBy, parmsNodeTypeUpdateCacheRemove.Replace("item.", ""),
		parms, parmsNoneType, cacheCond, whereCondi);

					bll_async_code += string.Format(@"
		async public static Task<{1}Info> GetItem{2}Async({4}) => await SqlHelper.CacheShellAsync(string.Concat(""{0}_BLL_{1}{2}_"", {3}), itemCacheTimeout, () => Select{7}.ToOneAsync());", solutionName, uClass_Name, cs[0].IsPrimaryKey ? string.Empty : parmsBy, parmsNodeTypeUpdateCacheRemove.Replace("item.", ""),
		parms, parmsNoneType, cacheCond, whereCondi);
		...
		public static Task<{0}Info> InsertAsync({1}) {{
			return InsertAsync(new {0}Info {{{2}}});
					...
					foreach (ColumnInfo cspk2 in table.Columns) {
						if (GetCSType(cspk2.Type) == "Guid?" && cspk2.IsPrimaryKey) {
						...
						if (GetCSType(cspk2.Type) == "DateTime?" && cspk2.Name == "create_time" ||
							GetCSType(cspk2.Type) == "DateTime?" && cspk2.Name == "update_time") {
...
", uClass_Name, "", redisRemove, cspk2GuidSetValue, cspk2GuidSetValuesss, table.Uniques.Count);
	...
	var fk_{1}s = {2}{0}.Select.ToList();", UFString(fk2[0].ReferencedTable.ClassName), LFString(fk2[0].ReferencedTable.ClassName), UFString(fk2[0].ReferencedTable.ClassName) == "User" ? solutionName + ".BLL." : "");
			...
			}}", GetCSType(fk2[0].ReferencedColumns[0].Type).Replace("?", ""), UFString(addname), addname);
