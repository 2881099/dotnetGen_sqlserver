...
> 不要认为 dbfirst low，使用 vs 建实体类和 navicat 建模型有区别吗？我们只是换了一个工具管理实体类与关系图而已。navicat 管理模型相当方便，同步好表结构后使用本工具一键就可以同步实体，以及各种规范的花式和你根本想不到的语法，默认还支持了缓存，支持数据库99%类型，避免很多重复劳动。
...
#### xx.db
	...
	修改数据库后，双击“./GenMs只更新db.bat”可快速覆盖，所有类都使用 partial，方便代码扩展亦不会被二次生成覆盖
